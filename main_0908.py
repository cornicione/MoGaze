...
class GazeNet (nn.Module):
    ...
    def calc_gaze_lola(self,head,eye):
        ...
        print("[calc_gaze]:gaze_lo : {}".format(gaze_lo))
        print("[calc_gaze]:gaze_la : {}".format(gaze_la))
...
def image_normalize(im_data, transform=None):
    ...
    im_data = cv2.resize(im_data, (input_image_size, input_image_size))
...
class GazeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        head_image_half_w = head_image_half_h = 200
        head_image = head_image[mid_x - head_image_half_w:mid_x + head_image_half_w,
                     mid_y - head_image_half_h:mid_y + head_image_half_h]

        # cv2.imshow("crop",head_image)
        # cv2.waitKey()
...
#8 gpus per node
    ...
    if online:
        pre_trained_model_path = model_dir + "train_1000_0908.pth"
    else:
        pre_trained_model_path = model_dir+"train_5000_step.pth"

#    FineTune = os.path.exists(pre_trained_model_path)
    FineTune = False

    print("Loading pre trained model path:{} and found model:{}".format(pre_trained_model_path, FineTune))
    if FineTune:
        model = GazeNet()
        if online:
            pre_trained_model = torch.load(pre_trained_model_path)['model']
        else:
...
# print parameters
    ...
    if online:
        ...
        train_loader, valid_loader = get_train_valid_loader(dataset, 1)
    ...
    #将真值也做归一化,和模型输出量纲相同
            ...
            # add weight (eye:3)
#            total_loss = loss_head + 3*loss_eye + loss_gaze
            ...
            if (step+1)%1000==0:
                ...
                torch.save({"model":model.module.state_dict(),"optim":optimizer.state_dict()},\
                            model_dir+"train_{}_0908.pth".format(1+step))
                for i in range(10):
                    gc.collect()
            total_loss.backward()
            optimizer.step()
            ...
            # code problem
                ...
                # add weight (eye:3)
                valid_total.append(loss_head + 3 * loss_eye + loss_gaze)
            print("head: {:.5f} eye: {:.5f} gaze: {:.5f} total: {:.5f}"\
            ...
            if online:
                ...
                train_loader, valid_loader = get_train_valid_loader(dataset, 128)
            dataiterator = iter(train_loader)
...
if online:
    model_dir = "/home/mc/ckpt/0.1/"
    data_dir = "/data/mc_data/MC4/"
    result_dir = "/home/mc/Result/"
else:
    # model_dir = "/home/momenta/Desktop/"            #
    # data_dir = '/home/momenta/Desktop/'
    # result_dir = "/home/momenta/Desktop/"
    model_dir = "./Model/"            #
    data_dir = './data/'
    result_dir = model_dir

if train:
    gaze_set = GazeDataset(data_dir + 'train',"train",transform)
    main(gaze_set)
    ...
    epoch = 1000
    output_path = result_dir + "test_pred_{}_{}.txt".format(date, epoch)
    model_path = model_dir + "train_{}_0908.pth".format(epoch)
